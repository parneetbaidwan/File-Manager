...
{
    // - Match screenshot vibe: "Saturday, January 10, 2026 01:14:07"
    // - If time is missing/invalid, show blank
    if (t <= 0) return "";
...
{
    // - Matches the screenshot vibe (File / Edit / View)
    // - On macOS, "Window" and "Help" are handled by the system menu bar
...
{
    // - Ensures shortcuts work even if the menu bar isn't focused
    // - On macOS, Ctrl in labels typically maps to Cmd visually/behaviorally
...
void MainFrame::DoOpen()
{
    ...
    // - Files: launch default application
...
{
    // Double-click acts like "Open..."
    DoOpen();
}

// ---------------- Menu handlers (HelloWorld style) ----------------

void MainFrame::OnMenuNew(wxCommandEvent&)     { DoNew(); }
void MainFrame::OnMenuOpen(wxCommandEvent&)    { DoOpen(); }
void MainFrame::OnMenuRename(wxCommandEvent&)  { DoRename(); }
void MainFrame::OnMenuDelete(wxCommandEvent&)  { DoDelete(); }

void MainFrame::OnMenuCopy(wxCommandEvent&)    { DoCopy(false); }
void MainFrame::OnMenuCut(wxCommandEvent&)     { DoCopy(true); }
void MainFrame::OnMenuPaste(wxCommandEvent&)   { DoPaste(); }

void MainFrame::OnMenuRefresh(wxCommandEvent&) { DoRefresh(); }
